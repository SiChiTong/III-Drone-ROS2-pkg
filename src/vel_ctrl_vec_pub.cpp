...
//Messages are sent based on a timed callback.
	public:
		VelocityControlVectorAdvertiser() : Node("vel_ctrl_vect_advertiser") {
			...
			cable_yaw_subscription_ = this->create_subscription<iii_interfaces::msg::PowerlineDirection>( 
				"cable_yaw_angle",	10,
				[this](iii_interfaces::msg::PowerlineDirection::ConstSharedPtr _msg) {
					cable_yaw_ = _msg->angle;
			});
	...
	private:
		...
		rclcpp::Subscription<iii_interfaces::msg::PowerlineDirection>::SharedPtr cable_yaw_subscription_;
...
void VelocityControlVectorAdvertiser::VelocityDroneControl(float xv, float yv, float zv){
	...
	vel_ctrl_vect.z = 0;
	vel_ctrl_vect.yaw = 0;
	vel_ctrl_vect.yawspeed = cable_yaw_; //0;
...
void VelocityControlVectorAdvertiser::OnDepthMsg(const sensor_msgs::msg::PointCloud2::SharedPtr _msg){
	...
	if(pcl_size > 0){
		// VelocityDroneControl(0.5, 0.0, 0.0);
		// return;
